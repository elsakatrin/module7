...
const TestSlice = ({ slice }) => (
  ...
  <section className={styles.contentwrapper}>
    ...
    {"<Imgix>"} is a component for React from react-imgix library. It generates an SrcSet {"(source set)"} from the image which grants the browser the flexibility to choose the best suited image for the current window size, so it makes the images responsive!
    So the SrcSet provides different versions of the image to the browser which the browser will select based on a few things; screen size and resolution, device pixel ratio {"(DPR)"}, sometimes bandwidth and the sizes prop from the developer.
    Since Imgix does this automatically, we have to spend a little less time worrying about making images responsive. Clever and comfortable! And of course there is the Imgixparams prop that we use to get a better control of the images.  
    </p>
        ...
        <p className={styles.paragraph}>All the images have the same sizes attribute, the sizes are used for making them responsive. We're pairing media queries with lengths and telling our browser where the breakpoints are for our website.</p>
 ...
 <div className={styles.paragraphwrapper}>
   ...
   <div className={styles.paragraphwrapper}>
    ...
    <p className={styles.paragraph}> 
    Here I attempted add parameters to control the image. PrismicNextImage wants us to use the width and height prop, here it's set to 600x401.
    ...
    The fit:"crop" parameter resizes the image to fit width and height or the aspect ratio parameter {"(ar:)"} and crops any excess image data. The result is supposed to match the width and height or aspect ratio without distorting the image.
    The width and height prop from NextPrismicImage can not be overwritten and doesn't allow the imgix paramerter to work as it should. If we remove width and height prop from the image, it goes out of control and becomes huge because then there aren't any size restrictions applied on the image.  
    </p>
  </div>
    ...
    {/* 3. Prismic next compress + attemt to resize */}
      <div className={styles.paragraphwrapper}>
        <p className={styles.parameters}>Imgix Parameters:</p>
          ...
          <p className={styles.paragraph}> 
            Here we're using a different fit parameter. Here I changed the width and height prop on the photo to 600x600 and kept on using fit:"crop" parameter. Without that parameter the image would become distorted. 
          </p>
          </div>
      <PrismicNextImage 
      ...
      <p className={styles.paragraph}> 
        On this image the compression has been removed and auto:"format" has been added instead. The format logic will attempt to serve images in AVIF format when it's supported, if it's not it falls back to WebP, if that's not supported either the fallback is JPG. 
        ...
        The quality {"(q:)"} has been changed to 80, when an image is set to compress the default quality is set to 45, but with the compress removed the default becomes 75.
        The image has been sharpened using unsharp mask {"(usm:)"} which sharpens the image details, and unsharp mask ratio {"(usmrad:)"} which determines how mnay pixels should be included to enhance the contrast when using unsharp mask. 
        Finally, a colorspace {"(cs:)"} has been added. When images are compressed they lose their colourspace and tend to look gray. Even though this photo isn't compressed anymore I wanted to be sure that the image would be served in the right colourspace. 
      </p>
      </div>
...
{/* NextPrismicImage conclution */}
    <div className={styles.paragraphwrapper}>
      <p className={styles.paragraph}> 
      Conclusion: The photos look good! Obviously, the imgixparams sizing parameters that we tried to use above didn't work. 
      You might have noticed that the images aren't responsive. I could have added CSS media queries or even layout="responsive" to the images. 
      My problem was that the sizes attribute and srcset don't apply as effortlessly to those as to the Imgix component.
...
<div className={styles.paragraphwrapper}>
      <p className={styles.parameters}>Imgix Parameters:</p>
        ...
        <p className={styles.paragraph}>
          This image has compression and the color space has been removed. This is added to mimic what the photo looks like when it's compressed without the API being altered.
        </p>
    </div>
  ...
  {/* 2. Imgix with parameters */}
      ...
      {/* 4. Imgix with DPR */}
        ...
        <p className={styles.paragraph}> 
          Can you see the quality difference on this one? Device pixel ratio or the DPR as added to this image. It controls the output density of your image. 
          ...
          But because this parameter is on this photo, it would always display as DRP:"2". That wouldn't be good for all devices, like a phone or very large screens. 
          I recommend opening this image in a new tab, changing the dpr="" in the url and see the difference when you change it!  
          Higher DPR makes the picture bigger so the sizes attribute had to be changed. 
        </p>
    ...
    {/* 5.Imgix with enhance */}
      <div className={styles.paragraphwrapper}>
        ...
        <div className={styles.paragraphwrapper}>
      <p className={styles.paragraph}> 
      Conclusion: Imgix has a lot of fun parameters, a lot more than I have put in here. Once I got the hang of it I really saw the practicality of it.  
      I think that the srcset in the imgix component are very comfortable to use and a clever technology. It's possible to use with other image components and make the scrsets manually, but the imgix component does it for us, which is super nice. 
      Also using the imgix component with Prismic is very convenient, because the client doesn't have to think about what size picture they upload, the code and the srcsets does it for them. 
      </p>
  </div>
</div>
